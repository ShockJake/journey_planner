...
@TestPropertySource(properties = """
        service.implementation.route.generation=no-opt
        service.implementation.route.optimization=no-opt
        """)
public class ABNoOptRouteOptimizationBenchmarkTest extends OptimizationTestBase {
    @Autowired
    private MockMvc mockMvc;
    @Autowired
    private RouteDataInit routeDataInit;
    private boolean shouldDoWarmUp = true;
    private static final String OUTPUT_PATH = "route_optimization_no_opt_" + Instant.now().getEpochSecond() + ".csv";
    ...
    void shouldOptimize10Routes(int tasksNumber) {
        // given
        var action = getRequestAction(mockMvc, request, status().isOk());
